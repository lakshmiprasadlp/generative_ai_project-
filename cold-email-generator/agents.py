...
# Set up the LLM model using Groq API
#model = "groq/llama-3.2-3b-preview"
"""llm = ChatGroq(
    model=model,
    verbose=True,
    temperature=0,
    groq_api_key=os.getenv('GROQ_API_KEY')  # Use the API key from environment variable)
"""
llm=ChatOpenAI(
    model_name="gpt-3.5-turbo",
    temperature=0,verbose=True,
    openai_api_key=os.getenv('OPENAI_API_KEY')  # Use the API key from environment variable
)




# Create Agent for Researcher
researcher = Agent(
